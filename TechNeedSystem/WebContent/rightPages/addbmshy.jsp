<%@ page language="java" contentType="text/html; charset=utf-8"
    ...
    <body>
     ...
     <jsp:useBean id="md5" class="bean.MD5JM" scope="page" />
        ...
        String bmyhm="",bmmm="",glbm="";
        bmyhm=request.getParameter("bmyhm");
        bmmm=request.getParameter("bmmm");
        if(bmmm!=null)
        {
        	bmmm=md5.transMD5(bmmm);
        }
        glbm=request.getParameter("glbm");
        String sql="insert into mgtInfo values('"+bmyhm+"','"+bmmm+"','"+glbm+"')";
        
      //  System.out.println(sql);
        String submit = request.getParameter("bmok");
    	if (submit != null && !submit.equals("")) {
    		String uname = request.getParameter("bmyhm");
			...
			try
			{
				...
				else
				{
		    		...
		    		if (i == 1) {
		    			//从iframe中跳出到父级页面
		    			out.println("<script language='javaScript'>"+
								"layer.open({title:'提示',content:'添加部门审核员成功！',skin: 'layui-layer-lan',closeBtn: 0,btn:['确定'],btn1:function(){window.location.href='yhgl.jsp'}});"
								+"</script>");
		    		}
		    		else {
		    			out.println("<script language='javaScript'> alert('添加部门审核员失败！');history.back();</script>");
			...
			{
				out.println("<script language='javaScript'> alert('添加部门审核员失败,有异常！');history.back();</script>");
			}
    		db.close();
    	}
       %>
